...
async function getUsernamebyEmail(email) {
  try {
    const user = await db.query("SELECT username FROM users WHERE email = ?", [
      email,
    ]);
    return user[0];
  } catch (error) {
...
async function addRefreshToken(userID, refreshToken) {
  try {
    const user = await db.query(
      "INSERT INTO refreshtokens (user_id, refresh_token) VALUES (? , ?);",
      [userID, refreshToken]
    );
    return user[0];
  } catch (error) {
...
async function getUserfromRefreshToken(token) {
  const user1 = await getRefreshToken(token)
    .then(async (userID) => {
