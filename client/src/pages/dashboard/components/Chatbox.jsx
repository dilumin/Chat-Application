...
function Chatbox() {
  ...
  const getMessagesFromDB = async () => {
    try {
      const res = await axiosInstance.post('/getAllMessages', { "myEmail": MyEmail, "fEmail": selectedFriend });
  ...
  useEffect(() => {
    if (socket) {
      socket.on('newMessage', (data) => {
        ...
        console.log("SELECTED FRIEND IS", selectedFriend, "   DATA SENDER IS", data.sender);
        if (data.sender === selectedFriend) {
          setMessages(prevMessages => [...prevMessages, { me: "", msg: data.message }]);
        }
      });
    }
    ...
    <div className="w-screen flex flex-col justify-center items-center h-screen bg-gray-100">
      <div className="h-5/6 w-3/4 bg-gray-200 p-6 flex flex-col justify-between rounded-lg shadow-lg overflow-hidden">
        <div className="flex flex-col space-y-4 overflow-y-auto p-4" style={{ maxHeight: 'calc(100% - 60px)' }}>
          ...
          <div ref={messagesEndRef} /> {/* This div ensures auto-scrolling to bottom */}
        </div>
        <Input />
      </div>
    </div>
